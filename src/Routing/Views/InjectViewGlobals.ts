	...
	public async handle(context: RequestContextContract): Promise<any> {
		...
		viewManager.injectViewGlobal(
			'route', <T extends keyof ApplicationRouteAttributeObject>(
				routeStr: T,
				attributes?: Partial<ApplicationRouteAttributeObject[T]> | any
			) => RouteHelper.urlFor(routeStr, attributes)
		);
		viewManager.injectViewGlobal(
		...
		viewManager.injectViewGlobal('isDevelopment', () => config('App').isDev());
