 ...
 * Package was out of date.... no way a pull req was going through any time soon
 *
...
export class SimpleCrypto {
	 ...
	 */
	public static generateRandomWordArray(length: number = 128): lib.WordArray {
	...
	private _decrypt(): PlainData {
		...
		const salt: any = enc.Hex.parse(this._dataBuffer.substr(0, 32))
		const initialVector: any = enc.Hex.parse(this._dataBuffer.substr(32, 32))
		const encrypted: string = this._dataBuffer.substring(64, this._dataBuffer.length-64)

		const key: string | lib.WordArray = PBKDF2(this._secret.toString(), salt, {
	...
	private _encrypt(): string {
		const salt: string | lib.WordArray = SimpleCrypto.generateRandom(128, true)
		const initialVector: string | lib.WordArray = SimpleCrypto.generateRandom(128, true)

		const key: lib.WordArray = PBKDF2(this._secret.toString(), salt, {
			keySize: this._keySize / 32,
			iterations: this._iterations
		})

		const encrypted: any = AES.encrypt(this._dataBuffer, key, {
	 ...
	 * @return  {string}	The decrypted data of the encrypted string.
	 */
	public decrypt(cipher: CipherText, encoder: any): PlainData
	 ...
	 * @return  {string}	The decrypted data of the encrypted string.
	 */
	public decrypt(cipher: CipherText, expectsObject: boolean, encoder: any): PlainData

	public decrypt(
		cipher?: CipherText,
		secondArg?: boolean | any,
		thirdArg?: boolean | any
	): PlainData {
		const setDecryptionOption = (arg: boolean | any): void => {
	 ...
	 * @return  {SimpleCrypto}		Current SimpleCrypto instance.
	 */
	public setEncoder(encoder: any): SimpleCrypto {
		/*
